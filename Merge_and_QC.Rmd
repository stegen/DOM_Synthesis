...
# check
fsetdiff(data[, .(`Calibrated m/z`)], mol[, .(`Calibrated m/z`)]) # should be zero

print(paste0("removed ", mz_to_remove_from_mol[, .N], " non-matching m/z values."))
...
# cleanup
rm(factors, temp, temp.data, curr.data, w, i)
...
rm(bad.mass.formulas, DBE.C.formulas, DBE.O.formulas, filters.per.formula, formula.occurence, formula.to.drop, HC.formulas, OC.formulas, p.formulas, singletons)

# comment this out in case you want to keep the original data tables
rm(data, mol, sample.richness)
...
# cleanup 
rm(factors.trim, temp, temp.data, curr.data, w, i)
...
# clean up
rm(info, out, temp, df, CHEMICAL_ELEMENTS)
...
# quick check
sum(is.na(data.trim)) # should be zero at this point
